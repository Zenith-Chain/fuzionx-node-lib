...
class FuzionXKit {
	constructor(
		...
		this.apiUrl = opts.apiURL || 'https://fuzionx.io/api';
	 ...
	 * @return {object} A JSON object with the keys pairs(information on each symbol-pair such as tick_size, min/max price, and min/max size) and currencies(array of all currencies involved in FuzionX)
	 */
	getConstants() {
		return createRequest(
			'GET',
	...
	/**
	 * Connect to FuzionX websocket and listen to an event
	...
	/**
	 * Disconnect from FuzionX websocket
	 */
	disconnect() {
		if (this.wsConnected()) {
	...
	/**
	 * Subscribe to FuzionX websocket events
	 * @param {array} events - The events to listen to
	 */
	subscribe(events = []) {
		if (this.wsConnected()) {
	...
	/**
	 * Unsubscribe to FuzionX websocket events
